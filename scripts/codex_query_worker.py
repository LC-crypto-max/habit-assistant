...
def sanitize_raw_evidence(value: Any) -> dict[str, Any]:
    ...
    safe: dict[str, Any] = {
        ...
        "llm_provider": sanitize(value.get("llm_provider") or "", 40),
        "analysisChannel": sanitize(value.get("analysisChannel") or value.get("analysis_channel") or "", 40),
        "llm_latency_ms": 0,
...
def enrich_public_url_items(items: list[dict[str, Any]], cfg: WorkerConfig | None = None) -> list[dict[str, Any]]:
...
def require_xiaohongshu_agent_reach_success(
        task: dict[str, Any], items: list[dict[str, Any]]) -> None:
    platform = sanitize(task.get("platform") or "", 80).lower()
    if platform != "xiaohongshu":
        return
    for item in items:
        if not isinstance(item, dict):
        ...
        if isinstance(raw, dict) and str(raw.get("agentReachStatus") or "").upper() == "SUCCESS":
            return
    raise RuntimeError(
        "Agent Reach could not read the Xiaohongshu note. OpenCLI is required to reuse the "
        "authorized visible Chrome tab without exporting cookies; open the exact note and verify "
        "that the opencli command and browser extension are installed.")


def require_llm_success(decisions: list[PolicyDecision], provider: str) -> None:
    failed = []
    for decision in decisions:
        if not decision.llm_required:
            continue
        raw = decision.item.get("rawMetadata") or decision.item.get("rawEvidence") or {}
        if not isinstance(raw, dict) or str(raw.get("llm_status") or "").upper() != "SUCCESS":
            failed.append(decision.item)
    if failed:
        raw = failed[0].get("rawMetadata") or failed[0].get("rawEvidence") or {}
        detail = sanitize(raw.get("llm_error") if isinstance(raw, dict) else "", 240)
        suffix = f" Detail: {detail}" if detail else ""
        raise RuntimeError(
            f"{provider.upper()} analysis did not return valid JSON; no fallback placeholder was ingested. "
            "Check the reported detail, provider authentication, model configuration, and retry."
            + suffix)
...
def run_once(cfg: WorkerConfig) -> int:
    ...
    try:
        ...
        if cfg.analysis_channel == "agent-reach":
            raw_items = enrich_public_url_items(collected_items, cfg)
            verbose_json(cfg.verbose, "Agent Reach enriched items", raw_items)
            require_xiaohongshu_agent_reach_success(task, raw_items)
        else:
            raw_items = collected_items
            verbose_json(cfg.verbose, "Public metadata items", raw_items)
        for item in raw_items:
            if not isinstance(item, dict):
                continue
            raw = item.get("rawMetadata") or item.get("rawEvidence")
            if not isinstance(raw, dict):
                raw = {}
            raw["analysisChannel"] = cfg.analysis_channel
            item["rawMetadata"] = raw
            item["rawEvidence"] = raw
        raw_items = enrich_content_for_llm(raw_items, verbose=cfg.verbose)
        verbose_json(cfg.verbose, "Content enriched items", raw_items)
        policy_gate = PolicyGate(sanitize)
        decisions = policy_gate.evaluate_all(raw_items)
        if cfg.verbose:
            policy_gate.print_traces(decisions)
        provider = cfg.analysis_provider
        command = cfg.trae_command if provider == "trae" else cfg.codex_command
        llm_gateway = WorkerLLMGateway(
            ...
            sanitizer=sanitize,
            provider=provider,
            max_steps=cfg.trae_max_steps,
        )
        llm_gateway.submit_policy_decisions(decisions)
        llm_gateway.wait()
        require_llm_success(decisions, provider)
...
def parse_args() -> WorkerConfig:
    ...
    parser.add_argument("--analysis-provider", choices=("codex", "trae"), default="codex",
                        help="CLI used for semantic analysis. Defaults to codex.")
    parser.add_argument("--analysis-channel", choices=("agent-reach", "public-metadata"), default="agent-reach",
                        help="Evidence channel used before semantic analysis.")
    parser.add_argument("--codex-command", default="codex", help="Codex CLI command. Defaults to 'codex'.")
    parser.add_argument("--trae-command", default="trae-cli", help="TRAE CLI command. Defaults to 'trae-cli'.")
    parser.add_argument("--trae-max-steps", type=int, default=8, help="Maximum TRAE agent steps per analysis.")
    ...
    return WorkerConfig(
        ...
        analysis_channel=args.analysis_channel,
        codex_command=args.codex_command,
        trae_command=args.trae_command,
        trae_max_steps=max(1, min(args.trae_max_steps, 50)),
